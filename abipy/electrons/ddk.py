...
from abipy.boltztrap import BoltztrapResult, BoltztrapResultRobot
...
class TransportFile(AbinitNcFile, Has_Header, Has_Structure, Has_ElectronBands):
    ...
    def get_boltztrap_result(self,itemp=None,tmesh=None):
        ...
        Args:
            itemp: the index of the temperature from which to create the Bolztrap result class
        """
        reader = self.reader
        ...
        dos = dos[0]
        vvdos = vvdos[:,:,0]
        return BoltztrapResult(wmesh,dos,vvdos,self.fermi,tmesh,self.volume,self.nelect,tau_temp=tau_temp,nsppol=1,margin=0.1)

    def get_boltztrap_results(self,tmesh=None):
        """
        Return multiple instances of Boltztrap results from the data in the TRANSPORT.nc file
        """
        results = [self.get_boltztrap_result(tmesh=tmesh,itemp=itemp) for itemp in [None]+list(range(self.ntemp))]
        return BoltztrapResultRobot(results)
